...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  void initState() {
    ...
    if (widget.from != null) {
      ...
      final isFromAuth =
          widget.from == 'biometric' ||
          widget.from == 'pin' ||
          widget.from == 'cancel_initial';

      if (isFromAuth) {
        debugPrint(
          'LoginScreen: Detected auth source in initState, preparing for forced redirect',
        );
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ref
              .read(routerNotifierProvider.notifier)
              .prepareForForcedLoginRedirect();
        });
      }
    }
  }
  ...
  Future<void> _signIn() async {
    ...
    try {
      ...
      if (wasFromAuth) {
        debugPrint(
          'LoginScreen: Detected login after forced sign-out from ${widget.from}, preparing redirect.',
        );
        ref
            .read(routerNotifierProvider.notifier)
            .prepareForForcedLoginRedirect();
        ...
        debugPrint(
          'LoginScreen: Regular sign-in, not from auth screen. From: ${widget.from ?? "null"}',
        );
      }
      ...
      if (wasFromAuth) {
        final target = ref
            .read(routerNotifierProvider.notifier)
            .debugGetPostAuthTarget();
        debugPrint(
          'LoginScreen: After successful login, PostAuthTarget = $target',
        );
      }

      // The router will automatically handle redirection based on auth state
      debugPrint(
        'LoginScreen: Sign in successful, router will handle redirect.',
      );
    } catch (e) {
        ...
        child: SingleChildScrollView(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                // Logo and title
                Icon(AppIcons.pallet, size: 80, color: context.primaryColor),
                ...
                // Help link (per UI/UX Guidelines Section 11)
                Align(
                ...
                // Sign in button (56dp height per UI/UX Guidelines)
                SizedBox(
                  height: 56,
