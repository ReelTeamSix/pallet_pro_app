...
enum PostAuthNavigationTarget { none, homeAfterForcedLogin }
...
class RouterNotifier extends Notifier<void> implements Listenable {
  ...
  PostAuthNavigationTarget _postAuthTargetValue = PostAuthNavigationTarget.none;

  // Getter and setter with debug logging
  PostAuthNavigationTarget get _postAuthTarget => _postAuthTargetValue;
  set _postAuthTarget(PostAuthNavigationTarget value) {
    if (_postAuthTargetValue != value) {
      debugPrint(
        'RouterNotifier: _postAuthTarget changing from $_postAuthTargetValue to $value',
      );
  ...
  void build() {
    ...
    ref.listen<AsyncValue<User?>>(authControllerProvider, (previous, next) {
      ...
      if (userJustSignedOut) {
        debugPrint(
          'RouterNotifier: User signed out, resetting initial auth/resume flags.',
        );
        // Explicitly preserve _postAuthTarget when signing out
        final targetBeforeSignOut = _postAuthTarget;
        debugPrint(
          'RouterNotifier: Preserving post-auth target during sign-out: $targetBeforeSignOut',
        );
        ...
        debugPrint(
          'RouterNotifier: User signed in, current postAuthTarget: $targetBeforeSignIn',
        );
    ...
    ref.listen<AsyncValue<UserSettings?>>(userSettingsControllerProvider, (
      previous,
      next,
    ) {
    ...
    _splashTimeoutTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      ...
      if (timeOnSplash > _maxSplashWaitTime) {
        debugPrint(
          'RouterNotifier: Splash timeout check - forcing redirect after ${timeOnSplash.inSeconds} seconds',
        );
  ...
  void _handleProviderChange<T>(
    ...
    final isSignOutEvent =
        providerName == 'AuthController' &&
    ...
    if (isSignOutEvent) {
      ...
      debugPrint(
        'RouterNotifier: Sign-out detected, immediately notifying. Preserving postAuthTarget: $currentTarget',
      );
      ...
      Future.microtask(() {
        debugPrint(
          'RouterNotifier: Force immediate redirect for sign-out. PostAuthTarget: $currentTarget',
        );
        ...
        if (_postAuthTarget != savedTarget) {
          debugPrint(
            'RouterNotifier: Restoring postAuthTarget after sign-out redirect: $savedTarget',
          );
    ...
    if (isSignInEvent) {
      ...
      final currentTarget = _postAuthTarget;
      debugPrint(
        'RouterNotifier: Sign-in detected. Current postAuthTarget: $currentTarget',
      );
    }
    ...
    if (providerName == 'UserSettingsController') {
      ...
      if (_isOnSettingsScreen) {
        debugPrint(
          'RouterNotifier: Ignoring UserSettings change while on settings screen',
        );
        return;
      }
    }
    ...
    if (isLoading != wasLoading ||
        ...
        hasValue != hadValue) {
      debugPrint(
        'RouterNotifier: $providerName changed significantly, considering refresh.',
      );
      ...
      if (timeSinceLastNotification < const Duration(milliseconds: 100)) {
        debugPrint(
          'RouterNotifier: Skipping notification, too soon after last one (${timeSinceLastNotification.inMilliseconds}ms)',
        );
        return;
      }
      ...
      Future.delayed(const Duration(milliseconds: 150), () {
        if (_postAuthTarget != savedTarget &&
            savedTarget != PostAuthNavigationTarget.none) {
          debugPrint(
            'RouterNotifier: Restoring postAuthTarget after state change: $savedTarget',
          );
          _postAuthTarget = savedTarget;
        }
      });
  ...
  void appResumed() {
    if (!kIsWeb && (Platform.isAndroid || Platform.isIOS)) {
      ...
      if (_lastAuthCompletionTime != null &&
        ...
        debugPrint(
          'RouterNotifier: App Resumed within cooldown period, ignoring.',
        );
        return;
      }
  ...
  void markInitialAuthCompleted() {
    ...
    _wasResumed = false;
    debugPrint(
      'RouterNotifier: Also reset resume flag to prevent redirect loops.',
    );
  ...
  void prepareForForcedLoginRedirect() {
    debugPrint(
      'RouterNotifier: Preparing for post-forced-sign-out redirect to home.',
    );
    _postAuthTarget = PostAuthNavigationTarget.homeAfterForcedLogin;
  }

  /// Resets the post-auth target if it was set, e.g., after login failure.
  void resetPostAuthTarget() {
    if (_postAuthTarget != PostAuthNavigationTarget.none) {
      debugPrint(
        'RouterNotifier: Resetting post-auth target due to login failure or cancellation.',
      );
      _postAuthTarget = PostAuthNavigationTarget.none;
    }
  }

  /// Debug method to get the current post auth target value.
  PostAuthNavigationTarget debugGetPostAuthTarget() {
  ...
  String? _redirectLogic(BuildContext context, GoRouterState state) {
    ...
    final isFromAuth =
        from == 'biometric' || from == 'pin' || from == 'cancel_initial';
    if (isFromAuth && location == '/login') {
      debugPrint(
        'RouterNotifier: Login screen has auth source query parameter: $from',
      );
      if (_postAuthTarget == PostAuthNavigationTarget.none) {
        debugPrint(
          'RouterNotifier: Setting postAuthTarget to homeAfterForcedLogin from query param',
        );
        _postAuthTarget = PostAuthNavigationTarget.homeAfterForcedLogin;
      }
    }
    ...
    try {
      ...
      if (recoveryToken != null) {
        debugPrint(
          'RouterNotifier: Password recovery token found. Redirecting.',
        );
      ...
      final splashWaitedTooLong =
          DateTime.now().difference(_appStartTime) > _maxSplashWaitTime;
      if (splashWaitedTooLong && isSplash) {
        debugPrint(
          'RouterNotifier: Auth initialization timeout. Forcing redirect to login.',
        );
      ...
      if (rawAuthError != null) {
        debugPrint(
          'RouterNotifier: Raw Auth Error: $rawAuthError. Redirecting to /login.',
        );
      ...
      if (isAuthActionLoading && !isSplash) {
        if (isLoginOrSignupRoute && from != 'timeout') {
          debugPrint(
            'RouterNotifier: Auth action in progress. Redirecting to splash.',
          );
      ...
      if (isSplash && from == 'auth_action' && isAuthActionLoading) {
        debugPrint(
          'RouterNotifier: Already on splash for auth action. Staying.',
        );
        return null;
      }
      ...
      if (!isLoggedIn) {
        ...
        debugPrint(
          'RouterNotifier: Executing 4a (Not Logged In). Location: $location, Allowed Public: $isAllowedPublicRoute',
        );
        ...
        debugPrint(
          'RouterNotifier: Not logged in, on allowed route ($location). Staying.',
        );
        _initialAuthDone = false;
        _wasResumed = false;
        return null;
      ...
      debugPrint(
        'RouterNotifier: Logged In as ${user.id}. Checking settings...',
      );
      final isSettingsLoading =
          !settingsState.hasValue && !settingsState.hasError;
      ...
      if (isSplash && isSettingsLoading) {
        debugPrint(
          'RouterNotifier: Already at splash waiting for settings. Staying.',
        );
        return null;
      }
      if (settingsError != null) {
        debugPrint('RouterNotifier: Settings Error: $settingsError. Redirecting to /splash to retry/wait.');
        // Don't log out the user on settings error. Redirect to splash to allow retry or manual recovery.
        return isSplash ? null : '/splash?from=settings_error';
      }
      ...
      if (userSettings == null) {
        if (isSplash || isLoginOrSignupRoute) {
          debugPrint(
            'RouterNotifier: Settings null (race?). Staying on waiting screen.',
          );
          return null;
        }
        debugPrint(
          'RouterNotifier: Settings null unexpectedly. Redirecting to /splash.',
        );
      ...
      debugPrint(
        'RouterNotifier: Checking for forced login redirect. PostAuthTarget: $_postAuthTarget',
      );

      // Set the target if we're on login with from=biometric or from=pin, even if not already set
      if ((from == 'biometric' || from == 'pin' || from == 'cancel_initial') &&
          isLoginOrSignupRoute &&
          _postAuthTarget == PostAuthNavigationTarget.none) {
        debugPrint(
          'RouterNotifier: On login with auth source, setting home redirect flag',
        );
        _postAuthTarget = PostAuthNavigationTarget.homeAfterForcedLogin;
      }

      if (_postAuthTarget == PostAuthNavigationTarget.homeAfterForcedLogin) {
        debugPrint(
          'RouterNotifier: REDIRECTING - Post-forced-sign-out login detected, going directly to home.',
        );
        _postAuthTarget =
            PostAuthNavigationTarget.none; // Consume the target state
        _initialAuthDone =
            true; // Mark initial auth as complete for this session
      ...
      debugPrint(
        'RouterNotifier: Logged In & Settings Loaded | Onboarding: ${userSettings.hasCompletedOnboarding} | UseBio: ${userSettings.useBiometricAuth} | UsePIN: ${userSettings.usePinAuth} | InitialAuthDone: $_initialAuthDone | WasResumed: $_wasResumed',
      );

      final isAuthRelatedRoute =
          isLoginOrSignupRoute ||
          isOnboardingRoute ||
      ...
      String? authRoute;
      String authReason = '';
      final bool bioAvailableAndEnabled =
          canUseBiometrics && (userSettings.useBiometricAuth);
      final bool pinEnabled = userSettings.usePinAuth;
      final bool isResumeCheck = _wasResumed && isOnProtectedLocation;
      ...
      final bool isInitialLaunchCheck =
          !_initialAuthDone && !shouldSkipInitialAuth;

      if (shouldSkipInitialAuth && !_initialAuthDone) {
        debugPrint(
          'RouterNotifier: Skipping initial auth check due to direct login or target flag',
        );
      ...
      // Only do initial launch check if NOT resuming and initial auth isn't done
        ...
        if (bioAvailableAndEnabled) {
          ...
          debugPrint(
            'RouterNotifier: Initial Launch: No bio/PIN enabled. Marking auth done.',
          );
          _initialAuthDone = true;
        }
      }
      ...
      if (needsAuthCheck && authRoute != null) {
        ...
        if (location == '/pin-auth' && from == 'biometric') {
          debugPrint(
            'RouterNotifier: Allowing navigation from biometric to PIN auth screen.',
          );
        ...
        if (location != authRoute) {
          debugPrint(
            'RouterNotifier: Redirecting to required auth route: $authRoute?reason=$authReason',
          );
        ...
        // If we need auth check AND ARE already on the correct auth route
          ...
          debugPrint(
            'RouterNotifier: Already on required auth route $authRoute for $reason. Staying.',
          );
      ...
      if (_initialAuthDone &&
          (isLoginOrSignupRoute || isOnboardingRoute || isSplash)) {
        debugPrint(
          'RouterNotifier: Initial auth complete, redirecting from $location to /home.',
        );
        return '/home?from=auth_complete';
      }

      // --- 10. Handle Setup Routes --- (After main auth flow)
      if (isBiometricSetupRoute &&
          (userSettings.useBiometricAuth || !canUseBiometrics)) {
        debugPrint(
          'RouterNotifier: On biometric setup but already setup/not applicable. Redirecting to /home.',
        );
        return '/home';
      }
      if (isPinSetupRoute &&
          (userSettings.pinHash != null && userSettings.pinHash!.isNotEmpty)) {
        debugPrint(
          'RouterNotifier: On PIN setup but PIN exists. Redirecting to /home.',
        );
        return '/home';
      }
      ...
      if (isPinAuthRoute && from == 'biometric') {
        debugPrint(
          'RouterNotifier: Allowing direct navigation from biometric to PIN auth',
        );
        return null;
      }
      // If user lands on auth route unexpectedly (not for initial/resume check)
      if (_initialAuthDone &&
          (isBiometricAuthRoute || isPinAuthRoute) &&
          reason != 'resume_auth' &&
          reason != 'initial_launch_auth') {
        debugPrint(
          'RouterNotifier: On auth route ($location) unexpectedly after auth complete. Redirecting to /home',
        );
        return '/home';
      }
      ...
      // --- Default Case ---
      debugPrint(
        'RouterNotifier: All checks passed for $location. Allowing navigation.',
      );
    ...
    ShellRoute(
      ...
      routes: [
        ...
        GoRoute(
          ...
          routes: [
            GoRoute(
              path: 'pallets/add',
              name: 'add_pallet',
              builder: (context, state) => const AddEditPalletScreen(),
            ),
            GoRoute(
              path: 'pallets/:id',
              name: 'pallet_detail',
              builder: (context, state) {
                ...
                return PalletDetailScreen(palletId: id);
              },
            ),
            GoRoute(
              path: 'items/:id',
        ...
        GoRoute(
          ...
          name: 'scan',
          builder: (context, state) =>
              const PlaceholderScreen(title: 'Scan/Add'),
        ),
        GoRoute(
          path: '/sales',
          name: 'sales',
...
class AppShell extends ConsumerWidget {
  ...
  void _navigate(
    ...
    Future.delayed(
      drawerIsOpen ? const Duration(milliseconds: 100) : Duration.zero,
      () {
              ...
              return ListView(
                ...
                children: [
                  DrawerHeader(
                    ...
                    child: Text(
                      'Pallet Pro',
                      style: Theme.of(drawerContext).textTheme.titleLarge
                          ?.copyWith(
                            color: Theme.of(
                              drawerContext,
                  ...
                  ListTile(
                    leading: Icon(
                      Icons.logout,
                      color: Theme.of(drawerContext).colorScheme.error,
                    ),
                    title: Text(
                      'Sign Out',
                      style: TextStyle(
              ...
              if (isError)
                ElevatedButton.icon(
                  onPressed: () {
                    ...
                    ref.refresh(userSettingsControllerProvider);
                  },
